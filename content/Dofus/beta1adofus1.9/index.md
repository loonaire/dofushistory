...
Je n'ai pas la date exacte de cette version mais j'en déduis quelle est sortie en mai 2004, je pense quelle est sortie le 12 [en me basant sur cette page](https://web.archive.org/web/20040624170449/http://forum.dofus.com:80/Autres/Divers,11). Je n'ai pas le changelog de cette version mais il est possible de récupérer quelques informations.  
...
On peux voir [cette capture](http://xenografter.chez-alice.fr/Screenshots/Bar.jpg) qui est réalisée sur les dernières versions de la beta (présence de pnj et de l'icone de bug à droite). [Cette capture](http://xenografter.chez-alice.fr/fin%20beta/Dolmen.jpg) montre le jeu juste avant la fin de la beta v17.2b. [Un autre screen](https://web.archive.org/web/20050119200759/http://www.dofus.com/bonus/forums/dofus_clubic01.jpg) montre l'interface des caractéristiques avec les métiers, ce screen a été fait entre la v15b et 1.0 mais je ne sais pas quand exactement.  
...
Du coté du client, l'interface de connexion et de choix de personnage est modifiée pour s'approcher des couleurs de l'interface de jeu (les interfaces sont visible [ici](https://web.archive.org/web/20040925001211/http://www.dofus.com:80/?page=decouvrir&rubrique=presentation&contenu=guide_pas01)), le nombre de personnages passe de 10 à 5, cette interface restera la même jusqu'à la version 1.13.1. Il y a également une modification sur l'interface de jeu, les icones en haut à droite sont désormais 3 et le bouton qui permettais de reporter un bug pendant la beta a disparu, l'interface de cette version est visible [ici](http://xenografter.chez-alice.fr/Screenshots/Chasse%20au%20Mulou.JPG) (la carte possède également quelques élément intéressant, il s'agit d'une des 2 mines de la foret abraknyde et elle possède un soleil qui a disparu depuis. Sur le screen on peux également voir le sort foudroiement qui est déja sorti).  

[Ce screen](https://web.archive.org/web/20221222195805/http://xenografter.chez-alice.fr/Screenshots/Fort%20Bobard.JPG) montre l'apparion des coffres aux trésor sur cette version.
...
Cette mise à jour est sortie le 20 juillet 2005, le changelog peux être consulté [ici](dofus/17140-modifications-version-1-8), pour les joueurs, elle n'ajoute rien. En revanche pour les développeurs, elle semble changer beaucoup de chose. En même temps que cette mise à jour sont sortie une nouvelle version du site (pour la version anglaise) ainsi que la partie communauté du forum.  
...
- Il deviens fortement conseillé de jouer via le dofus.exe plutot que par un navigateur internet. On note également qu'il n'est plus question de lancer le jeu depuis jouer.dofus.com, j'ignore quand a eu lieu cette modification mais cela veux dire beaucoup.
- Le système de connexion ne se fait plus directement vers le serveur de jeu mais il passe d'abord par un serveur de connexion, ce serveur permet d'arriver sur une page où l'on peux choisir son personnage et son serveur. Une interface secondaire est ajoutée au moment de la création du personnage où le joueur doit choisir le serveur. Cette interface restera tel quel jusqu'a dofus 1.13.1.  
